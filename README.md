...
5. Check result data in `result` dir
   - `data`: When you turn `ON` the `OUTPUT_CSV` in `Ce_MPS.h`, time courses of (x,y,z,vx,vy,vz) for each particle are output in this directory as .csv file
   - `wall`: When you turn `ON` the `OUTPUT_CSV` in `Ce_MPS.h`, time courses of the wall particle data are output as .csv file
...
1. Install PCL
   ...
   - The code is basically developed on the version 1.8.1 of PCL
2. Run the program without specifying mesh file
   - The simulation will stop (with a warning `file not found`) while creating `point.dat` in `result`
3. Change the parameters defined by `#define` in the file `makeMesh.cpp` in `PCL` directory
   - `PARTICLE_DISTANCE`: match with `Ce_MPS.h`
   - `WIDTH`・`HEIGHT`: match `WIDTH`×`HEIGHT` with the line number of `point.dat`, and match aspect ratio
...
5. Copy `point.dat` from `result` to `PCL/build` then run `./makeMesh`
   - `p_mesh.vtk` will be generated in `build` directory. Use this file for mesh file when you run
   
   ### Acknowlegement
   These programs are wrtitten by refering the Japanese book "Ryushihou Nyumon" (a primer for particle simulation) by Seiichi Koshizuka, Kazuya Shibata, and Kohei Murotani, Maruzen 2014 
